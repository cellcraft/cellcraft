...
from mcpi.block import Block, GLASS

# import other methods of cellcraft
from cellcraft.src.cellpack import add_cellpack
from cellcraft.src.protein import add_pdb
path = "./"
...
def minecraft_api(args):
    ...
    if args[1] == 'pdb':
        if args[6] == 'load':
            ...
            pickle.dump(
                (array, colordict, texture), open('cellcraft/cache/' + '_'.join(args[1:5])+".pkl", "wb"))
        else:
            array, colordict, texture = pickle.load(
                open('cellcraft/cache/' + '_'.join(args[1:5])+".pkl", "rb"))
        swap = True
    if args[1] == 'cellpack':
        swap = False
        if args[6] == 'load':
        ...
        if it[0] > 0:
            ...
            if swap:
                ...
                mc.setBlock(
                    ...
                    Block(int(texture[int(it[0])]), colordict[int(it[0])]))
            else:
                mc.setBlock(
